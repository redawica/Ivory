...
local items = {
	settingsfile = "DarhangeR_Druid_PvP.xml",
	{ type = "title", text = "Druid PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Druid_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Druid_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Druid PvP by DarhangeR for 3.3.5a", "Welcome to Druid PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Druid_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Druid_PvP_DarhangeR", queue, abilities);
end
