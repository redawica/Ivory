...
local items = {
	settingsfile = "DarhangeR_Hunter_PvP.xml",
	{ type = "title", text = "Hunter PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Hunter_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Hunter_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Hunter PvP by DarhangeR for 3.3.5a", "Welcome to Hunter PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Hunter_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Hunter_PvP_DarhangeR", queue, abilities);
end
