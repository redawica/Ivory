...
local items = {
	settingsfile = "DarhangeR_Enhancement_PvP.xml",
	{ type = "title", text = "Shaman Enhancement PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Enhancement_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Enhancement_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Shaman Enhancement PvP by DarhangeR for 3.3.5a", "Welcome to Shaman Enhancement PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Enhancement_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Enhancement_PvP_DarhangeR", queue, abilities);
end
