...
local items = {
	settingsfile = "DarhangeR_Mage_PvP.xml",
	{ type = "title", text = "Mage PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Mage_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Mage_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Mage PvP by DarhangeR for 3.3.5a", "Welcome to Mage PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Mage_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Mage_PvP_DarhangeR", queue, abilities);
end
