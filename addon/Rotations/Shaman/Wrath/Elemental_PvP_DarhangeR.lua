...
local items = {
	settingsfile = "DarhangeR_Elemental_PvP.xml",
	{ type = "title", text = "Shaman Elemental PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Elemental_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Elemental_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Shaman Elemental PvP by DarhangeR for 3.3.5a", "Welcome to Shaman Elemental PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Elemental_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Elemental_PvP_DarhangeR", queue, abilities);
end
