...
local items = {
	settingsfile = "DarhangeR_Rogue_PvP.xml",
	{ type = "title", text = "Rogue PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Rogue_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Rogue_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Rogue PvP by DarhangeR for 3.3.5a", "Welcome to Rogue PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Rogue_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Rogue_PvP_DarhangeR", queue, abilities);
end
