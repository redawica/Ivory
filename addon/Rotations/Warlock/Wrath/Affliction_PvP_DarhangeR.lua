...
local items = {
	settingsfile = "DarhangeR_Affliction_PvP.xml",
	{ type = "title", text = "Warlock Affliction PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Affliction_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Affliction_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Warlock Affliction PvP by DarhangeR for 3.3.5a", "Welcome to Warlock Affliction PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Affliction_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Affliction_PvP_DarhangeR", queue, abilities);
end
