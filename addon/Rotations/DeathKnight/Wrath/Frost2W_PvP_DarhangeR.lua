...
local items = {
	settingsfile = "DarhangeR_Frost2W_PvP.xml",
	{ type = "title", text = "Death Knight Frost2W PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Frost2W_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Frost2W_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Death Knight Frost2W PvP by DarhangeR for 3.3.5a", "Welcome to Death Knight Frost2W PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Frost2W_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Frost2W_PvP_DarhangeR", queue, abilities);
end
