...
data.UseConfiguredTrinkets = function(GetSetting, fallbackSpell, fallbackUnit)
	if not GetSetting then
		return false
	end
	local _, enabled = GetSetting("trinketenabled")
	if not enabled then
		return false
	end
	...
	local function getUnit(key)
		local v = GetSetting(key)
		if type(v) == "string" and v ~= "" then
			return v
		end
		return fallbackUnit or "target"
	end

	local function trySlot(slot, idKey, spellKey, unitKey)
		local neededId = getNumber(idKey)
		if neededId and GetInventoryItemID("player", slot) ~= neededId then
			return false
		end
		if not ni.player.slotcastable(slot) or ni.player.slotcd(slot) ~= 0 then
			return false
		end

		local spellId = getNumber(spellKey)
		local spellUnit = getUnit(unitKey)
		if spellId then
			if ni.spell.valid(spellUnit, spellId, false, true, true) then
				ni.player.useinventoryitem(slot)
				return true
			end
			return false
		end
		...
		if fallbackSpell then
			if ni.spell.valid(fallbackUnit or "target", fallbackSpell, false, true, true) then
				ni.player.useinventoryitem(slot)
				return true
			end
			return false
	...
	if trySlot(13, "trinket13id", "trinket13spell", "trinket13unit") then
		return true
	end
	if trySlot(14, "trinket14id", "trinket14spell", "trinket14unit") then
		return true
	end
	return false
end
