...
local items = {
	settingsfile = "DarhangeR_Subtlety_PvP.xml",
	{ type = "title", text = "Rogue Subtlety PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Subtlety_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Subtlety_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Rogue Subtlety PvP by DarhangeR for 3.3.5a", "Welcome to Rogue Subtlety PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Subtlety_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Subtlety_PvP_DarhangeR", queue, abilities);
end
