...
local items = {
	settingsfile = "DarhangeR_DeathKnight_PvP.xml",
	{ type = "title", text = "Death Knight PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("DeathKnight_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("DeathKnight_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Death Knight PvP by DarhangeR for 3.3.5a", "Welcome to Death Knight PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("DeathKnight_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("DeathKnight_PvP_DarhangeR", queue, abilities);
end
