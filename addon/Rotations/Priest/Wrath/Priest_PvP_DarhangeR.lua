...
local items = {
	settingsfile = "DarhangeR_Priest_PvP.xml",
	{ type = "title", text = "Priest PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Priest_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Priest_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Priest PvP by DarhangeR for 3.3.5a", "Welcome to Priest PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Priest_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Priest_PvP_DarhangeR", queue, abilities);
end
