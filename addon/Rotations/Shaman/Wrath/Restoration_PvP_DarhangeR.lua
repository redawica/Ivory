...
local items = {
	settingsfile = "DarhangeR_Restoration_PvP.xml",
	{ type = "title", text = "Shaman Restoration PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Restoration_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Restoration_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Shaman Restoration PvP by DarhangeR for 3.3.5a", "Welcome to Shaman Restoration PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Restoration_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Restoration_PvP_DarhangeR", queue, abilities);
end
