...
local items = {
	settingsfile = "DarhangeR_Shaman_PvP.xml",
	{ type = "title", text = "Shaman PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Shaman_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Shaman_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Shaman PvP by DarhangeR for 3.3.5a", "Welcome to Shaman PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Shaman_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Shaman_PvP_DarhangeR", queue, abilities);
end
