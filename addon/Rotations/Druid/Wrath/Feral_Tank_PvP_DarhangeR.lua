...
local items = {
	settingsfile = "DarhangeR_Feral_Tank_PvP.xml",
	{ type = "title", text = "Druid Feral Tank PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Feral_Tank_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Feral_Tank_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Druid Feral Tank PvP by DarhangeR for 3.3.5a", "Welcome to Druid Feral Tank PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Feral_Tank_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Feral_Tank_PvP_DarhangeR", queue, abilities);
end
