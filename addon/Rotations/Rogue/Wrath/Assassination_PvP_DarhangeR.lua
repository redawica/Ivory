...
local items = {
	settingsfile = "DarhangeR_Assassination_PvP.xml",
	{ type = "title", text = "Rogue Assassination PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Assassination_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Assassination_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Rogue Assassination PvP by DarhangeR for 3.3.5a", "Welcome to Rogue Assassination PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Assassination_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Assassination_PvP_DarhangeR", queue, abilities);
end
