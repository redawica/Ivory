...
local items = {
	settingsfile = "DarhangeR_Blood_DPS_PvP.xml",
	{ type = "title", text = "Death Knight Blood DPS PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Blood_DPS_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Blood_DPS_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Death Knight Blood DPS PvP by DarhangeR for 3.3.5a", "Welcome to Death Knight Blood DPS PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Blood_DPS_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Blood_DPS_PvP_DarhangeR", queue, abilities);
end
