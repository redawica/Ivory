...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Warrior PvP by DarhangeR for 3.3.5a", "Welcome to Warrior PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Warrior_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Warrior_PvP_DarhangeR", queue, abilities);
end
