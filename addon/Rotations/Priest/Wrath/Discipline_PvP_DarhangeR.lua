...
local items = {
	settingsfile = "DarhangeR_Discipline_PvP.xml",
	{ type = "title", text = "Priest Discipline PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Discipline_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Discipline_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Priest Discipline PvP by DarhangeR for 3.3.5a", "Welcome to Priest Discipline PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Discipline_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Discipline_PvP_DarhangeR", queue, abilities);
end
