...
local items = {
	settingsfile = "DarhangeR_Paladin_PvP.xml",
	{ type = "title", text = "Paladin PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Paladin_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Paladin_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Paladin PvP by DarhangeR for 3.3.5a", "Welcome to Paladin PvP profile.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Paladin_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Paladin_PvP_DarhangeR", queue, abilities);
end
