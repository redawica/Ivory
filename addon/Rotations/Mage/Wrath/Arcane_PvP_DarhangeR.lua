...
local items = {
	settingsfile = "DarhangeR_Arcane_PvP.xml",
	{ type = "title", text = "Mage Arcane PvP by |c0000CED1DarhangeR" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("Arcane_PvP_DarhangeR", items);
end

local function OnUnLoad()
	ni.GUI.DestroyFrame("Arcane_PvP_DarhangeR");
end
...
-----------------------------------
	["Window"] = function()
		if not popup_shown then
			ni.debug.popup("Mage Arcane PvP by DarhangeR for 3.3.5a", "Welcome to Mage Arcane PvP profile. Racials are configured for PvP mode.")
			popup_shown = true;
		end
	end,
}

ni.bootstrap.profile("Arcane_PvP_DarhangeR", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = { "Error" }
	local abilities = {
		["Error"] = function()
	...
	ni.bootstrap.profile("Arcane_PvP_DarhangeR", queue, abilities);
end
